...
def generate_title_from_message(msg: str) -> str:
    """Generate a clean, concise title from the initial message."""
...
async def chat_sync(
    ...
    """Synchronous chat endpoint returning complete response."""
    ...
    # 4. Retrieve RAG Document context
    retrieved_sources = []
    ...
    # 5. Check Calculator tool if appropriate
    tools_used = []
    ...
    # 9. Auto-extract implicit memory
...
async def chat_stream(
    ...
    """Real Server-Sent Events (SSE) streaming chat endpoint."""
    # 1. Resolve or create conversation
    is_new_conv = False
    if chat_in.conversation_id:
        conv = db.query(Conversation).filter(
    ...
    async def sse_event_stream():
        ...
        # Save assistant message to DB with a new session to avoid concurrency issues
...
async def chat_regenerate(
    ...
    """Remove the last assistant response and stream a freshly regenerated response."""
    conv = db.query(Conversation).filter(
